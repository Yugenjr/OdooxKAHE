...
const SHARED_TRIPS: SharedTrip[] = [
  {
    id: 1,
    title: 'Epic Paris & Rome Adventure',
    ...
    budget: 4500,
    views: 3240,
    description: 'An unforgettable 2-week journey through the most romantic cities in Europe',
  },
  {
    id: 2,
    title: 'Southeast Asia 3 Weeks',
    author: 'Mike Chen',
    destination: 'Southeast Asia',
    image: 'https://images.unsplash.com/photo-1537225228614-b4fad35b6789?q=80&w=800&auto=format&fit=crop',
    duration: 21,
    budget: 3200,
    views: 5120,
    description: 'Budget-friendly exploration of Thailand, Cambodia, and Vietnam',
  },
  {
    id: 3,
    ...
    budget: 3800,
    views: 2890,
    description: 'North and South Island adventure with hiking and scenic drives',
  },
  {
    id: 4,
  ...
  },
  {
    ...
    budget: 5500,
    views: 6780,
    description: 'Luxury island resort experience with water activities and relaxation',
  },
]
...
const SharedTripsPage: React.FC = () => {
  ...
  const handleCopyLink = (id: number) => {
    const link = `${window.location.origin}/shared-trip/${id}`
            ...
            >
              ...
              <div className="p-5">
                ...
                <p className="text-white/60 text-sm mb-4 flex items-center gap-1">
                  <User className="w-3 h-3" /> by {trip.author}
                </p>
          ...
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setSelectedTrip(null)}>
            <div className="bg-gray-900 rounded-2xl border border-white/10 max-w-2xl w-full p-8 max-h-96 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="text-white/60 mb-4 flex items-center gap-2">
                <User className="w-4 h-4" /> by {selectedTrip.author}
              </p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-white/5 p-4 rounded-lg">
                  ...
                  <div className="font-bold">{selectedTrip.destination}</div>
                </div>
                <div className="bg-white/5 p-4 rounded-lg">
                  <div className="text-white/60 text-sm mb-1">Duration</div>
                  <div className="font-bold">{selectedTrip.duration} days</div>
                </div>
                <div className="bg-white/5 p-4 rounded-lg">
                  <div className="text-white/60 text-sm mb-1">Budget</div>
                  <div className="font-bold">${selectedTrip.budget}</div>
                </div>
                <div className="bg-white/5 p-4 rounded-lg">
                  <div className="text-white/60 text-sm mb-1">Views</div>
                  <div className="font-bold">{selectedTrip.views}</div>
                </div>
              </div>

              <div className="flex gap-3">
                <button
                  onClick={() => handleCopyLink(selectedTrip.id)}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-white/10 hover:bg-white/20 text-white rounded-lg transition"
                >
